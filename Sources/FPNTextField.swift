...
open class FPNTextField: UITextField {
    ...
    private func setupLeftView() { //This Method Defination added by ashish
        leftView = UIView()
        leftViewMode = .always
        ...
        // Add flag button and phone code text field to leftView
        leftView?.addSubview(flagButton)
        leftView?.addSubview(phoneCodeTextField)

        // Vertical line view setup
        let verticalLine = UIView()
        verticalLine.backgroundColor = .lightGray// Set the color of the separator
        verticalLine.translatesAutoresizingMaskIntoConstraints = false
        leftView?.addSubview(verticalLine)

        // Set constraints for flag button
        ...
        // Set constraints for phone code text field
        ...
        // Set constraints for vertical line separator
        NSLayoutConstraint(item: verticalLine, attribute: .leading, relatedBy: .equal, toItem: phoneCodeTextField, attribute: .trailing, multiplier: 1, constant: 8).isActive = true
        NSLayoutConstraint(item: verticalLine, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 1).isActive = true
        NSLayoutConstraint(item: verticalLine, attribute: .top, relatedBy: .equal, toItem: leftView, attribute: .top, multiplier: 1, constant: 1).isActive = true
        NSLayoutConstraint(item: verticalLine, attribute: .bottom, relatedBy: .equal, toItem: leftView, attribute: .bottom, multiplier: 1, constant: -1).isActive = true

        // Adjust the trailing constraint of the leftView to accommodate the separator and any following elements
        NSLayoutConstraint(item: leftView!, attribute: .trailing, relatedBy: .equal, toItem: verticalLine, attribute: .trailing, multiplier: 1, constant: 8).isActive = true
    }
    /*{
        leftView = UIView()
        leftViewMode = .always
    ...
    @objc private func displayCountries() {
        ...
        case .list:
            (delegate as? FPNTextFieldDelegate)?.fpnDisplayCountryList(selectedCountryTextField: self.tag)
        }
    }
    ...
    private func fpnDidSelect(country: FPNCountry) {
        (delegate as? FPNTextFieldDelegate)?.fpnDidSelectCountry(name: country.name, dialCode: country.phoneCode, code: country.code.rawValue, selectedCountryField: self.tag)
    ...
    private func getWidth(text: String) -> CGFloat {
        if let font = phoneCodeTextField.font {
            ...
            return (size.width+20).rounded(.up)// + 20 added by ashish
        } else {
            phoneCodeTextField.sizeToFit()

            return (phoneCodeTextField.frame.size.width+20).rounded(.up) // + 20 added by ashish
        }
    }
        ...
        if let countryCode = selectedCountry?.code {
            do {
                ...
                if let inputString = formatter?.inputString(phoneNumber) {
                    placeholder = "Enter Phone Number"//remove(dialCode: "+\(example.countryCode.stringValue)", in: inputString)//Placeholder code added by ashish
                } else {
                    placeholder = nil
                }
            } catch _ {
